...
static unsigned char* serialize_int(unsigned char* buf, signed int a);
...
void sendCommand(int left, int right, int timeout_ms) {
    if(!isInitialized) {
        isInitialized = 1;
        initialize();
    }
    ...
    int retval = boost::asio::write(serial,boost::asio::buffer(buffer,12));
    if(retval <= 0)
